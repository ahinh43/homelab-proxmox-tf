...
#     host        = one(proxmox_virtual_environment_vm.main.ipv4_addresses[0])
#   }
#   provisioner "file" {
...
#     command = <<EOT
#     set -x
#     ssh -o ConnectTimeout=5 core@${one(proxmox_virtual_environment_vm.main.ipv4_addresses[0])} '(sleep 2; sudo reboot)&'; sleep 3
